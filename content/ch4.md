...
| **FIFO** | 最早进入 | 队列 | **✅ 有！** | 简单，但**有 Belady 异常** |
...
> **OPT、LRU 没有 Belady 异常，FIFO 有**。
> 因为 OPT 看未来、LRU 看过去活跃度，**驻留集随页框单调扩张**——多一个页框只会"多保留有用的页"。
...
> **被分了用不完 = 内部碎片**（分配单元内部浪费）
> **分不出去 = 外部碎片**（空闲单元间小到没法分）

### Ch4 易错点 Top 10

1. ❌ **物理地址停在表达式**：`9×4096+308` **必须算成 37172**——"答案不完整"会扣分
